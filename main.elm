...
3. Fulvous whistling duck  --  https://en.wikipedia.org/wiki/Fulvous_whistling_duck
4. Snow goose  --  https://en.wikipedia.org/wiki/Snow_goose
5. Ross's goose  --  https://en.wikipedia.org/wiki/Ross%27s_goose
6. Greater white-fronted goose  --  https://en.wikipedia.org/wiki/Greater_white-fronted_goose
7. Pink-footed goose  --  https://en.wikipedia.org/wiki/Pink-footed_goose
8. Brant (goose)  --  https://en.wikipedia.org/wiki/Brant_(goose)
9. Barnacle goose  --  https://en.wikipedia.org/wiki/Barnacle_goose
10. Cackling goose  --  https://en.wikipedia.org/wiki/Cackling_goose
11. Canada goose  --  https://en.wikipedia.org/wiki/Canada_goose
12. Mute swan  --  https://en.wikipedia.org/wiki/Mute_swan
13. Trumpeter swan  --  https://en.wikipedia.org/wiki/Trumpeter_swan
14. Tundra swan  --  https://en.wikipedia.org/wiki/Tundra_swan
...
-- search-updated birds list, filtered to keep only birds with at least 
-- a part of their name partially/fully matched by a work in search list
birdsOnSearch : Model -> BirdsList
birdsOnSearch model =
  birds |> List.filter (\((birdName,birdSpecies),_,_) ->  
             model.searchList |> List.any (\ wrd -> 
                                   (commonWrds wrd) && (
                                   ((String.toLower) birdName |> String.contains (String.toLower wrd)) ||
                                   ((String.toLower) birdSpecies |> String.contains (String.toLower wrd)) )))
               
-- carousel below search bar
carousel1 : Model -> Html Msg
carousel1 model =
  Carousel.config CarouselMsg  []
    |> Carousel.withIndicators
    |> Carousel.slides
       (birds |> List.map (\((birdName,birdSpecies),birdUrl,birdImage) ->             
                Slide.config [class "bg-secondary"] (image "250px" birdImage)
                  |> Slide.caption [  ]
                    [ Html.h2 [class "font-weight-bold text-white fixed-top"] [ Html.text birdName ] 
                    , Html.div [class "font-weight-bold "] [Html.text birdSpecies ]
                    ])  
       )
    |> Carousel.view model.carousel1State
   
-- accordion of search results
accordion1 : Model -> Html Msg
accordion1 model =
  Accordion.config AccordionMsg
    |> Accordion.withAnimation
    |> Accordion.cards
        (birdsOnSearch model |> List.map (\((birdName,birdSpecies),birdUrl,birdImage) ->
                      Accordion.card
                          { id = birdName
                          ...
                          , blocks =
                              [ Accordion.block [ Block.align Text.alignXsLeft ]
                                  [ Block.titleH4 [] [ Html.text ("Scientific name: " ++ birdSpecies) ]
                                  , Block.link [ href birdUrl 
                                               , target "_blank"] [ Html.text ("Wikipedia link: The " ++ birdName) ] ]
                              ]
                          }
                                          )
          )
    ...
    [ Grid.container [] -- Wrap in a container to center the navbar
        [ Navbar.config NavbarMsg
            ...
            |> Navbar.brand                     -- Add logo to your brand with a little styling to align nicely
                [ href "https://en.wikipedia.org/wiki/Gull" 
                , target "_blank"]
                [ Html.img
                    [ src "https://b.kisscc0.com/20180705/lzq/kisscc0-seabird-beak-gulls-drawing-bird-over-the-sea-5b3e5368e61136.2949116715308112409424.png"
            ...
            |> Navbar.items
                [ Navbar.itemLink
                    [ href "https://en.wikipedia.org/wiki/List_of_birds_by_flight_speed"  
                    , target "_blank"] [ Html.text "Fastest Avian Birds" ]
                , Navbar.itemLink
                    [ href "https://en.wikipedia.org/wiki/List_of_largest_birds"  
                    , target "_blank"] [ Html.text "Birds by Size" ]
                , Navbar.dropdown        
                    { id = "mydropdown"
                    , toggle = Navbar.dropdownToggle [] [ Html.text "Popular Ontario Birds" ]
                    , items =
            ...
            |> Navbar.customItems
                [ Navbar.formItem [ class "fixed-right" ]
                    [ Input.text [ Input.attrs [placeholder "search birds", value model.searchString, Events.onInput UpdateSearch ]]
                    , Button.button
                      [ Button.success
